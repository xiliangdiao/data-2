...
years = [f"{x:04d}" for x in range(450,500)]
ii=0
result = np.zeros(600)

for yy in years:
    for mm in months:
        data=nc.Dataset('/ihesp/user/xiliangdiao/ihespdata/ocn330-500/cmpr_B.E.13.B1850C5.ne120_t12.sehires38.003.sunway_02.pop.h.'+yy+'-'+mm +'.UVEL.nc')
...
##############################################
        ...
        result[ii] = streamSV
        ii = ii + 1
        np.savetxt('/scratch/user/xiliangdiao/plot/PM450-500test.out',result)